...
/**
  ...
  */
case class QuotReqGrpComponent(noRelatedSymField:NoRelatedSymField,
...
object QuotReqGrpComponent extends SfFixDecoder {
  ...
  def decode(flds: Seq[Tuple2[Int, Any]], startPos:Int = 0):Option[QuotReqGrpComponent] = {
    val (pos, myFields, nextTagPosLookup) = extractMyFieldsAndPopulatePositions(false, flds, startPos)
    validateMandatoryFieldsPresent(myFields)

    if (myFields.nonEmpty) {
      Some(QuotReqGrpComponent(NoRelatedSymField.decode(myFields.get(NoRelatedSymField.TagId)).get,
