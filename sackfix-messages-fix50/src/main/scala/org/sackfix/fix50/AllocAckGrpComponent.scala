...
/**
  ...
  */
case class AllocAckGrpComponent(noAllocsField:Option[NoAllocsField]=None,
...
object AllocAckGrpComponent extends SfFixDecoder {
  ...
  def decode(flds: Seq[Tuple2[Int, Any]], startPos:Int = 0):Option[AllocAckGrpComponent] = {
    val (pos, myFields, nextTagPosLookup) = extractMyFieldsAndPopulatePositions(false, flds, startPos)
    validateMandatoryFieldsPresent(myFields)

    if (myFields.nonEmpty) {
      Some(AllocAckGrpComponent(myFields.get(NoAllocsField.TagId).flatMap(f=>NoAllocsField.decode(f)),
