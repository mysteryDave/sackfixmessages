...
/**
  ...
  */
case class AllocGrpComponent(noAllocsField:Option[NoAllocsField]=None,
...
object AllocGrpComponent extends SfFixDecoder {
  ...
  def decode(flds: Seq[Tuple2[Int, Any]], startPos:Int = 0):Option[AllocGrpComponent] = {
    val (pos, myFields, nextTagPosLookup) = extractMyFieldsAndPopulatePositions(false, flds, startPos)
    validateMandatoryFieldsPresent(myFields)

    if (myFields.nonEmpty) {
      Some(AllocGrpComponent(myFields.get(NoAllocsField.TagId).flatMap(f=>NoAllocsField.decode(f)),
