...
/**
  ...
  */
case class ExecsGrpComponent(noExecsField:Option[NoExecsField]=None,
...
object ExecsGrpComponent extends SfFixDecoder {
  ...
  def decode(flds: Seq[Tuple2[Int, Any]], startPos:Int = 0):Option[ExecsGrpComponent] = {
    val (pos, myFields, nextTagPosLookup) = extractMyFieldsAndPopulatePositions(false, flds, startPos)
    validateMandatoryFieldsPresent(myFields)

    if (myFields.nonEmpty) {
      Some(ExecsGrpComponent(myFields.get(NoExecsField.TagId).flatMap(f=>NoExecsField.decode(f)),
