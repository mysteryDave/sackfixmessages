...
/**
  ...
  */
case class QuotReqLegsGrpComponent(noLegsField:Option[NoLegsField]=None,
...
object QuotReqLegsGrpComponent extends SfFixDecoder {
  ...
  def decode(flds: Seq[Tuple2[Int, Any]], startPos:Int = 0):Option[QuotReqLegsGrpComponent] = {
    val (pos, myFields, nextTagPosLookup) = extractMyFieldsAndPopulatePositions(false, flds, startPos)
    validateMandatoryFieldsPresent(myFields)

    if (myFields.nonEmpty) {
      Some(QuotReqLegsGrpComponent(myFields.get(NoLegsField.TagId).flatMap(f=>NoLegsField.decode(f)),
