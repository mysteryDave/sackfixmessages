...
/**
  ...
  */
case class ExecsGroup(execIDField:Option[ExecIDField]=None) extends SfFixGroup  with SfFixRenderable with SfFixFieldsToAscii {
...
object ExecsGroup extends SfGroupDecoder {
  ...
  override def isFirstField(tagId:Int) : Boolean = tagId==ExecIDField.TagId 
  ...
  def decodeSingle(flds: Seq[Tuple2[Int, Any]], startPos:Int):Tuple2[Int,Option[ExecsGroup]] = {
    if (!isFirstField(flds(startPos)._1)) (startPos, None)
      ...
      if (myFields.nonEmpty) {
        (pos,Some(ExecsGroup(myFields.get(ExecIDField.TagId).flatMap(f=>ExecIDField.decode(f)))))
      } else (startPos, None)
    }
  }
