...
case class QuoteSetsGroup(quoteSetIDField:QuoteSetIDField,
                          ...
                          quotEntryGrpComponent:Option[QuotEntryGrpComponent]=None) extends SfFixGroup  with SfFixRenderable with SfFixFieldsToAscii {
  ...
  def format( fmt: ((StringBuilder,SfFixRenderable)=>Unit), b:StringBuilder = new StringBuilder()): StringBuilder = {
    ...
    quotEntryGrpComponent.foreach(fmt(b,_))
    b
  }
    ...
    if (!isFirstField(flds(startPos)._1)) (startPos, None)
      ...
      if (myFields.nonEmpty) {
        (pos,Some(QuoteSetsGroup(QuoteSetIDField.decode(myFields.get(QuoteSetIDField.TagId)).get,
          ...
          QuotEntryGrpComponent.decode(flds, startPos))))
      } else (startPos, None)
    }
  }
