...
/**
  ...
  */
case class TrdAllocGrpComponent(noAllocsField:Option[NoAllocsField]=None,
...
object TrdAllocGrpComponent extends SfFixDecoder {
  ...
  def decode(flds: Seq[Tuple2[Int, Any]], startPos:Int = 0):Option[TrdAllocGrpComponent] = {
    val (pos, myFields, nextTagPosLookup) = extractMyFieldsAndPopulatePositions(false, flds, startPos)
    validateMandatoryFieldsPresent(myFields)

    if (myFields.nonEmpty) {
      Some(TrdAllocGrpComponent(myFields.get(NoAllocsField.TagId).flatMap(f=>NoAllocsField.decode(f)),
