...
/**
  ...
  */
case class PosUndInstrmtGrpComponent(noUnderlyingsField:Option[NoUnderlyingsField]=None,
...
object PosUndInstrmtGrpComponent extends SfFixDecoder {
  ...
  def decode(flds: Seq[Tuple2[Int, Any]], startPos:Int = 0):Option[PosUndInstrmtGrpComponent] = {
    val (pos, myFields, nextTagPosLookup) = extractMyFieldsAndPopulatePositions(false, flds, startPos)
    validateMandatoryFieldsPresent(myFields)

    if (myFields.nonEmpty) {
      Some(PosUndInstrmtGrpComponent(myFields.get(NoUnderlyingsField.TagId).flatMap(f=>NoUnderlyingsField.decode(f)),
