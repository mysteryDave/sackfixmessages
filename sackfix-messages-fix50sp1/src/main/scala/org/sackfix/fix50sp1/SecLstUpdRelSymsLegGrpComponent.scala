...
/**
  ...
  */
case class SecLstUpdRelSymsLegGrpComponent(noLegsField:Option[NoLegsField]=None,
...
object SecLstUpdRelSymsLegGrpComponent extends SfFixDecoder {
  ...
  def decode(flds: Seq[Tuple2[Int, Any]], startPos:Int = 0):Option[SecLstUpdRelSymsLegGrpComponent] = {
    val (pos, myFields, nextTagPosLookup) = extractMyFieldsAndPopulatePositions(false, flds, startPos)
    validateMandatoryFieldsPresent(myFields)

    if (myFields.nonEmpty) {
      Some(SecLstUpdRelSymsLegGrpComponent(myFields.get(NoLegsField.TagId).flatMap(f=>NoLegsField.decode(f)),
