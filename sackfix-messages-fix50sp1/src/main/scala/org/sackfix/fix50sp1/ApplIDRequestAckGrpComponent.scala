...
/**
  ...
  */
case class ApplIDRequestAckGrpComponent(noApplIDsField:Option[NoApplIDsField]=None,
...
object ApplIDRequestAckGrpComponent extends SfFixDecoder {
  ...
  def decode(flds: Seq[Tuple2[Int, Any]], startPos:Int = 0):Option[ApplIDRequestAckGrpComponent] = {
    val (pos, myFields, nextTagPosLookup) = extractMyFieldsAndPopulatePositions(false, flds, startPos)
    validateMandatoryFieldsPresent(myFields)

    if (myFields.nonEmpty) {
      Some(ApplIDRequestAckGrpComponent(myFields.get(NoApplIDsField.TagId).flatMap(f=>NoApplIDsField.decode(f)),
