...
/**
  ...
  */
case class InstrmtLegGrpComponent(noLegsField:Option[NoLegsField]=None,
...
object InstrmtLegGrpComponent extends SfFixDecoder {
  ...
  def decode(flds: Seq[Tuple2[Int, Any]], startPos:Int = 0):Option[InstrmtLegGrpComponent] = {
    val (pos, myFields, nextTagPosLookup) = extractMyFieldsAndPopulatePositions(false, flds, startPos)
    validateMandatoryFieldsPresent(myFields)

    if (myFields.nonEmpty) {
      Some(InstrmtLegGrpComponent(myFields.get(NoLegsField.TagId).flatMap(f=>NoLegsField.decode(f)),
