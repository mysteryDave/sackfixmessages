...
/**
  ...
  */
case class SecLstUpdRelSymGrpComponent(noRelatedSymField:Option[NoRelatedSymField]=None,
...
object SecLstUpdRelSymGrpComponent extends SfFixDecoder {
  ...
  def decode(flds: Seq[Tuple2[Int, Any]], startPos:Int = 0):Option[SecLstUpdRelSymGrpComponent] = {
    val (pos, myFields, nextTagPosLookup) = extractMyFieldsAndPopulatePositions(false, flds, startPos)
    validateMandatoryFieldsPresent(myFields)

    if (myFields.nonEmpty) {
      Some(SecLstUpdRelSymGrpComponent(myFields.get(NoRelatedSymField.TagId).flatMap(f=>NoRelatedSymField.decode(f)),
