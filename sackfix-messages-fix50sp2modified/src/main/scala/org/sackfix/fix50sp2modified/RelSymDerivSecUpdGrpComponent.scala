...
/**
  ...
  */
case class RelSymDerivSecUpdGrpComponent(noRelatedSymField:Option[NoRelatedSymField]=None,
...
object RelSymDerivSecUpdGrpComponent extends SfFixDecoder {
  ...
  def decode(flds: Seq[Tuple2[Int, Any]], startPos:Int = 0):Option[RelSymDerivSecUpdGrpComponent] = {
    val (pos, myFields, nextTagPosLookup) = extractMyFieldsAndPopulatePositions(false, flds, startPos)
    validateMandatoryFieldsPresent(myFields)

    if (myFields.nonEmpty) {
      Some(RelSymDerivSecUpdGrpComponent(myFields.get(NoRelatedSymField.TagId).flatMap(f=>NoRelatedSymField.decode(f)),
