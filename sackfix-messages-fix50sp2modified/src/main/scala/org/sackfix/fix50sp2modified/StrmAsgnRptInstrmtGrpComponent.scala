...
/**
  ...
  */
case class StrmAsgnRptInstrmtGrpComponent(noRelatedSymField:Option[NoRelatedSymField]=None,
...
object StrmAsgnRptInstrmtGrpComponent extends SfFixDecoder {
  ...
  def decode(flds: Seq[Tuple2[Int, Any]], startPos:Int = 0):Option[StrmAsgnRptInstrmtGrpComponent] = {
    val (pos, myFields, nextTagPosLookup) = extractMyFieldsAndPopulatePositions(false, flds, startPos)
    validateMandatoryFieldsPresent(myFields)

    if (myFields.nonEmpty) {
      Some(StrmAsgnRptInstrmtGrpComponent(myFields.get(NoRelatedSymField.TagId).flatMap(f=>NoRelatedSymField.decode(f)),
