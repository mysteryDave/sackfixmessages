...
/**
  ...
  */
case class StrmAsgnRptGrpComponent(noAsgnReqsField:Option[NoAsgnReqsField]=None,
...
object StrmAsgnRptGrpComponent extends SfFixDecoder {
  ...
  def decode(flds: Seq[Tuple2[Int, Any]], startPos:Int = 0):Option[StrmAsgnRptGrpComponent] = {
    val (pos, myFields, nextTagPosLookup) = extractMyFieldsAndPopulatePositions(false, flds, startPos)
    validateMandatoryFieldsPresent(myFields)

    if (myFields.nonEmpty) {
      Some(StrmAsgnRptGrpComponent(myFields.get(NoAsgnReqsField.TagId).flatMap(f=>NoAsgnReqsField.decode(f)),
