...
/**
  ...
  */
case class RelSymDerivSecGrpComponent(noRelatedSymField:Option[NoRelatedSymField]=None,
...
object RelSymDerivSecGrpComponent extends SfFixDecoder {
  ...
  def decode(flds: Seq[Tuple2[Int, Any]], startPos:Int = 0):Option[RelSymDerivSecGrpComponent] = {
    val (pos, myFields, nextTagPosLookup) = extractMyFieldsAndPopulatePositions(false, flds, startPos)
    validateMandatoryFieldsPresent(myFields)

    if (myFields.nonEmpty) {
      Some(RelSymDerivSecGrpComponent(myFields.get(NoRelatedSymField.TagId).flatMap(f=>NoRelatedSymField.decode(f)),
